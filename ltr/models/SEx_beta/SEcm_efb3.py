...
class SEcmnet(nn.Module):
    ...
    def __init__(self, feature_extractor, neck_module, head_module, used_layers,
        ...
        self.feat_adjust = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(24, 64, kernel_size=1, stride=1, padding=0),
                nn.BatchNorm2d(64),
                nn.ReLU()
            ),
...
@model_constructor
def SEcm_efb3(backbone_pretrained=True,used_layers=('layer3',),pool_size=None,unfreeze_layer3=False,use_NL=True):
    # backbone
    # backbone_net = backbones.mobilenet_v2(pretrained=backbone_pretrained)
    backbone_net = backbones.efficientnet('efficientnet-b3')
