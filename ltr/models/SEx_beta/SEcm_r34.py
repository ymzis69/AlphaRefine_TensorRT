...
from ltr.models.neck.PtCorr_r34_trt import PtCorr as Corr_r34_trt
...
class SEcmnet(nn.Module):
    ...
    def forward_ref(self, train_imgs, train_bb):
        ...
        train_feat_dict = self.extract_backbone_features(train_imgs.view(-1, *train_imgs.shape[-3:]), convert_trt=False)  # `OrderedDict{'layer3':Tensor}`
    ...
    def forward_test(self, test_imgs, mode='train', branches=['corner'], convert_trt=False):
        ...
        if not convert_trt:
            ...
            test_feat_dict = self.extract_backbone_features(test_imgs.view(-1, *test_imgs.shape[-3:]),
                                                            layers=['conv1', 'layer1', 'layer2', 'layer3'], convert_trt=False)
            ...
            backbone_r34_256_trt = torch2trt(self.extract_backbone_features, [test_imgs.view(-1, *test_imgs.shape[-3:])])
            Lfeat_list = backbone_r34_256_trt(test_imgs.view(-1, *test_imgs.shape[-3:]))
            torch.save(backbone_r34_256_trt.state_dict(), 'trt_models/trt_model_RF_RF_r34/backbone_r34_256_trt.pth')
        ...
        if mode == 'train':
            ...
            if not convert_trt:
                ...
                torch.save(corner_head_trt.state_dict(), 'trt_models/trt_model_RF_RF_r34/corner_head_trt.pth')
                ...
                torch.save(feat_adjust_0_trt.state_dict(), 'trt_models/trt_model_RF_RF_r34/feat_adjust_0_trt.pth')

                feat_adjust_1_trt = torch2trt(self.feat_adjust[1], [Lfeat_list[1]])
                Lfeat_list1 = feat_adjust_1_trt(Lfeat_list[1])
                torch.save(feat_adjust_1_trt.state_dict(), 'trt_models/trt_model_RF_RF_r34/feat_adjust_1_trt.pth')

                feat_adjust_2_trt = torch2trt(self.feat_adjust[2], [Lfeat_list[2]])
                Lfeat_list2 = feat_adjust_2_trt(Lfeat_list[2])
                torch.save(feat_adjust_2_trt.state_dict(), 'trt_models/trt_model_RF_RF_r34/feat_adjust_2_trt.pth')
                ...
                torch.save(mask_head_trt.state_dict(), 'trt_models/trt_model_RF_RF_r34/mask_head_trt.pth')
                print('**********************TensorRT model 6 convert complete**********************')
                print('**********************resnet34 model convert complete**********************')
        else:
    ...
    def extract_backbone_features(self, im, layers=None, convert_trt=True):
        if layers is None:
            layers = self.used_layers
        return self.feature_extractor(im, layers, convert_trt)
...
def SEcm_resnet34(backbone_pretrained=True, used_layers=('layer3',), pool_size=None, unfreeze_layer3=False):
    ...
    # neck
    neck_net = Corr_r34_trt(pool_size=pool_size)
