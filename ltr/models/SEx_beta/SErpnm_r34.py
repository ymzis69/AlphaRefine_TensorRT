...
#rom thop import profile
#rom thop import clever_format
#mport torch
...
@model_constructor
def SEcm_resnet34(backbone_pretrained=False, used_layers=('layer3',), pool_size=None, unfreeze_layer3=False):
    # backbone
    ...
    return net
#net = SEcmnet(feature_extractor=backbones.ResNet(backbones.BasicBlock(), [3, 4, 6, 3], output_layers=['default'], dilation_factor=1),neck_module=Corr(),
#head_module=(rpn.RPNHead(inplanes=64),mask.Mask_Predictor_fine()),
#used_layers=('layer3'),extractor_grad=True,unfreeze_layer3=False).eval().cuda()
#net = ResNet(Bottleneck, [3, 4, 6, 3], output_layers=['default'])
#net = resnet34(pretrained=True)
#x = torch.randn(1,3,256,256).cuda()
#tensor_x = Variable(torch.unsqueeze(x, dim=0).float(), requires_grad=False)
#y = torch.randn(1,3,256,256).cuda()
#z = torch.randn(4)
#tensor_y = Variable(torch.unsqueeze(y, dim=0).float(), requires_grad=False)
#tensor_y = Variable(torch.unsqueeze(x, dim=0).float(), requires_grad=False)
#flops, params = profile(net,inputs=(tensor_x,tensor_y,z))
