...
from ltr.models.neck import CorrNL,Depth_Corr,Naive_Corr,CorrNL_efb0_trt
...
class SEcmnet(nn.Module):
    ...
    def __init__(self, feature_extractor, neck_module, head_module, used_layers,
        ...
        self.feat_adjust = nn.ModuleList([
            ...
            nn.Sequential(
                nn.Conv2d(40, 512, kernel_size=1, stride=1, padding=0),
                nn.BatchNorm2d(512),
                nn.ReLU()
            )
        ])
    ...
    def forward_test(self, test_imgs, mode='train', convert_trt=False):
        ...
        if not convert_trt:
            ...
            backbone_efb0_256_trt = torch2trt(self.extract_backbone_features, [test_imgs.view(-1, *test_imgs.shape[-3:])])
            Lfeat_list = backbone_efb0_256_trt(test_imgs.view(-1, *test_imgs.shape[-3:]))
            torch.save(backbone_efb0_256_trt.state_dict(), 'trt_models/trt_model_RF_RF_efb0/backbone_efb0_256_trt.pth')
        ...
        if mode=='train':
            ...
            if not convert_trt:
                ...
                torch.save(corner_head_trt.state_dict(), 'trt_models/trt_model_RF_RF_efb0/corner_head_trt.pth')
                ...
                torch.save(feat_adjust_0_trt.state_dict(), 'trt_models/trt_model_RF_RF_efb0/feat_adjust_0_trt.pth')

                feat_adjust_1_trt = torch2trt(self.feat_adjust[1], [Lfeat_list[1]])
                Lfeat_list1 = feat_adjust_1_trt(Lfeat_list[1])
                torch.save(feat_adjust_1_trt.state_dict(), 'trt_models/trt_model_RF_RF_efb0/feat_adjust_1_trt.pth')

                feat_adjust_2_trt = torch2trt(self.feat_adjust[2], [Lfeat_list[2]])
                Lfeat_list2 = feat_adjust_2_trt(Lfeat_list[2])
                torch.save(feat_adjust_2_trt.state_dict(), 'trt_models/trt_model_RF_RF_efb0/feat_adjust_2_trt.pth')
                ...
                torch.save(mask_head_trt.state_dict(), 'trt_models/trt_model_RF_RF_efb0/mask_head_trt.pth')
                print('**********************TensorRT model 6 convert complete**********************')
                print('**********************efficientnetb0 model convert complete**********************')
        else:
...
@model_constructor
def SEcm_efb0(backbone_pretrained=True,used_layers=('layer3',),pool_size=None,unfreeze_layer3=False,use_NL=True):
    # backbone
    # backbone_net = backbones.mobilenet_v2(pretrained=backbone_pretrained)
    backbone_net = backbones.efficientnet('efficientnet-b0')

    # neck
    neck_net = CorrNL_efb0_trt.CorrNL(pool_size=pool_size, use_NL=use_NL)
