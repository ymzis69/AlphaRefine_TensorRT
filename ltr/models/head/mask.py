...
class Mask_Predictor_fine(nn.Module):
    ...
    #     return out
    def forward(self, corr_feat, Lfeat0, Lfeat1, Lfeat2):
        '''corr_feat是经过correlation和Non-local处理后的特征, Lfeat是backbone提取出的底层特征'''
        '''corr_feat: (batch,64,16,16)
           Lfeat: (batch,64,128,128), (batch,256,64,64), (batch,512,32,32)'''
        out = self.post0(F.interpolate(self.h2(corr_feat),size=(32,32),mode='bilinear')+self.v2(Lfeat2)) # (b,32,32,32)+(b,32,32,32) --> (b,16,32,32)
        out = self.post1(F.interpolate(self.h1(out),size=(64,64),mode='bilinear')+self.v1(Lfeat1)) # (b,16,64,64)+(b,16,64,64) --> (b,4,64,64)
        out = self.post2((F.interpolate(self.h0(out), size=(128, 128),mode='bilinear') + self.v0(Lfeat0)).contiguous())
